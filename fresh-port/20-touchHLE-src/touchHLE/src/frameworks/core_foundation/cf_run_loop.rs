...
use crate::frameworks::foundation::ns_run_loop::{run_run_loop, run_run_loop_single_iteration};
...
/// `void CFRunLoopRun(void)` — run the current thread's run loop "forever"
/// (until stopped / it runs out of sources). MoleWorld's loading path drives an
/// `NSTimer` (which calls `-[LoadingScene begainLoad]`) and relies on the run
/// loop actually running to fire it. touchHLE previously had no `CFRunLoopRun`,
/// so the dyld fallback turned it into a no-op — the run loop never ran, the
/// timer never fired, and loading never started. We forward to the same
/// `run_run_loop` that backs `-[NSRunLoop run]`, which fires timers and (on the
/// main thread) pumps events/compositing, returning when the loop has nothing
/// left to do.
fn CFRunLoopRun(env: &mut Environment) {
    let current_run_loop = CFRunLoopGetCurrent(env);
    run_run_loop(env, current_run_loop, /* single_iteration: */ false, None);
}
